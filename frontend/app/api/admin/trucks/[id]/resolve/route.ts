...
// POST - Résoudre une panne de camion
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3001/admin/trucks/${truckId}/resolve`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Erreur lors de la résolution de la panne' },
